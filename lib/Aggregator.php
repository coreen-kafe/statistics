	...
	public function getConsent() {
		...
		$parsed = array();
			...
			if (!preg_match('/STAT/', $logline)) continue;
		        if (!preg_match('/User/', $logline)) continue;

			if(preg_match('/successfully authenticated/', $logline)) {
                            $authArray['type'] = 'authen';
			    $date = trim(substr($logline, 0, 16));
			    $authArray['date'] = $date;
			    preg_match_all('/\'(.*?)\'/', $logline, $authenticated, PREG_PATTERN_ORDER);				
			    for($i = 0; $i < count($authenticated[0]); $i++) {
				    $decAttr = base64_decode($authenticated[0][$i]);
				    $decVal = SimpleSAML\Utils\Crypto::aesDecrypt($decAttr);

				    if($i == 0) $authArray['user'] = $decVal;
				    if($i == 1) $authArray['addr'] = $decVal;
			    }
			    array_push($parsed, $authArray);
			} else if(preg_match('/made a consent/', $logline)) {
			    $authArray['type'] = 'consent';
			    $date = trim(substr($logline, 0, 16));
			    $authArray['date'] = $date;
			    preg_match_all('/\'(.*?)\'/', $logline, $consented, PREG_PATTERN_ORDER);
			    for($i =0; $i < count($consented[0]); $i++) {
				if(preg_match('/http/', $consented[0][$i])) {
				    $authArray['service'] = $consented[0][$i];
				} else {
				    $decAttr = base64_decode($consented[0][$i]);
				    $decVal = SimpleSAML\Utils\Crypto::aesDecrypt($decAttr);
				    if($i == 1) $authArray['user'] = $decVal;
                    		    if($i == 2) $authArray['addr'] = $decVal;
				}
			    }
			    array_push($parsed, $authArray);
			}
